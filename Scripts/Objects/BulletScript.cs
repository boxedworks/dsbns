...
namespace Assets.Scripts.Objects
{
  public class BulletScript : MonoBehaviour
  {
      ...
      bool TakeDamage(ActiveRagdoll r)
      {
          ...
          ))
        {
          ...
          if (_hitAmount <= _penatrationAmount)
          {
            ...
            // Check smart bullets
            if ((_sourceDamageRagdoll._PlayerScript?.HasPerk(Perk.PerkType.SMART_BULLETS) ?? false) && _sourceType != ItemManager.Items.FLAMETHROWER)
            {
    ...
    void Deflect(ItemScript redirectorItem, bool recoil = false)
    {
      ...
      // Check special
      if (redirectorItem._ragdoll._IsPlayer && redirectorItem._ragdoll._PlayerScript.HasPerk(Perk.PerkType.EXPLOSIVE_PARRY))
        _explodeOnHide = true;
    }
  }
}