...
namespace Assets.Scripts.Objects.CustomEntities
{
  public class CustomObstacle : MonoBehaviour
  {
    ...
    // Perks
    static Dictionary<int, List<System.Tuple<Perk.PerkType, int>>> _BUYABLE_PERK_TIERS;
    System.Tuple<Perk.PerkType, int> _perk_info;
    ...
    static void CheckInit()
    {
      ...
      {
        _BUYABLE_PERK_TIERS = new Dictionary<int, List<System.Tuple<Perk.PerkType, int>>>();

        _BUYABLE_PERK_TIERS.Add(0, new List<System.Tuple<Perk.PerkType, int>>());
        _BUYABLE_PERK_TIERS[0].Add(System.Tuple.Create(Perk.PerkType.LASER_SIGHTS, 75));

        _BUYABLE_PERK_TIERS.Add(1, new List<System.Tuple<Perk.PerkType, int>>());
        _BUYABLE_PERK_TIERS[1].Add(System.Tuple.Create(Perk.PerkType.EXPLOSION_RESISTANCE, 250));
        _BUYABLE_PERK_TIERS[1].Add(System.Tuple.Create(Perk.PerkType.EXPLOSIONS_UP, 350));
        _BUYABLE_PERK_TIERS[1].Add(System.Tuple.Create(Perk.PerkType.AKIMBO, 400));

        _BUYABLE_PERK_TIERS.Add(2, new List<System.Tuple<Perk.PerkType, int>>());
        _BUYABLE_PERK_TIERS[2].Add(System.Tuple.Create(Perk.PerkType.ARMOR_UP, 600));
        _BUYABLE_PERK_TIERS[2].Add(System.Tuple.Create(Perk.PerkType.MAX_AMMO_UP, 500));
        _BUYABLE_PERK_TIERS[2].Add(System.Tuple.Create(Perk.PerkType.FASTER_RELOAD, 500));
        _BUYABLE_PERK_TIERS[2].Add(System.Tuple.Create(Perk.PerkType.SMART_BULLETS, 650));
      }
    ...
    public void Init(bool midgame = false)
    {
      ...
      else if (use_type == InteractType.BUYPERK)
      {
        ...
        _text.text = $"buy {_perk_info.Item1}: {_pointCost}\n<color=yellow>{Perk._PERK_DESCRIPTIONS[_perk_info.Item1]}</color>";
    ...
    public void Interact(PlayerScript p, InteractSide side)
    {
      ...
      if (SurvivalManager.HasPoints(p._Id, _pointCost))
      {
        ...
        if (use_type == InteractType.REMOVEBARRIER)
        {
          ...
          // Play sound
          p._Ragdoll.PlaySound("Survival/Buy_Wall", 0.9f, 1.1f);
        }
        else if (use_type == InteractType.BUYITEM)
        {
          ...
          var hasAkimbo = Perk.HasPerk(p._Id, Perk.PerkType.AKIMBO);
          ...
          // Play sfx
          p._Ragdoll.PlaySound("Survival/Buy_Weapon", 0.9f, 1.1f);
        ...
        else if (use_type == InteractType.BUYUTILITY)
        {
          ...
          if (side == InteractSide.DEFAULT)
          {
            ...
            {
              // Check utility cap
              if (equipment._UtilitiesRight.Length == ShopHelper._Utility_Cap[utility_type])
              {
                ...
                {
                  // Check utility cap
                  if (equipment._UtilitiesLeft.Length == ShopHelper._Utility_Cap[utility_type])
                  {
    ...
    System.Tuple<bool, ActiveRagdoll.Side, string> CheckEquip(PlayerScript p, ItemManager.Items item_type, InteractSide side, bool akimbo, int equipmentIndex)
    {
      ...
      var has_actual_two_handed = (item_type_l != ItemManager.Items.NONE && ShopHelper.IsActuallyTwoHanded(item_type_l)) ||
        (item_type_r != ItemManager.Items.NONE && ShopHelper.IsActuallyTwoHanded(item_type_r));
      ...
      if (side == InteractSide.DEFAULT)
      {
        ...
        else if (ShopHelper.IsActuallyTwoHanded(item_type))
          if (actual_two_handed_type == item_type)
            return System.Tuple.Create(false, equip_side, text_haveAlready);
          else
        ...
        {
          if (ShopHelper.IsTwoHanded(item_type_r) && ShopHelper.IsTwoHanded(item_type))
          {
            ...
            else if (akimbo && !ShopHelper.IsTwoHanded(item_type_l))
              equip_side = ActiveRagdoll.Side.LEFT;
          }
          else if (ShopHelper.IsTwoHanded(item_type_l) && ShopHelper.IsTwoHanded(item_type))
          {
            if (item_type_l == item_type)
            {
              if (item_type_r != item_type)
            ...
            else if (akimbo && !ShopHelper.IsTwoHanded(item_type_r))
        ...
        {
          if (ShopHelper.IsTwoHanded(item_type_l) && ShopHelper.IsTwoHanded(item_type))
          {
            if (item_type_l == item_type)
              if (akimbo)
                return System.Tuple.Create(true, ActiveRagdoll.Side.RIGHT, text);
      ...
      else if (side == InteractSide.LEFT)
      {
        ...
        else if (has_item_l && has_item_r)
        {
          ...
          else if (ShopHelper.IsTwoHanded(item_type_r) && ShopHelper.IsTwoHanded(item_type))
          {
            if (akimbo)
              return System.Tuple.Create(true, ActiveRagdoll.Side.LEFT, text);
            else
        ...
        {
          if (ShopHelper.IsTwoHanded(item_type))
          {
            if (ShopHelper.IsTwoHanded(item_type_r))
            {
              if (akimbo)
                return System.Tuple.Create(true, ActiveRagdoll.Side.LEFT, text);
              else
      ...
      else
      {
        ...
        else if (has_item_l && !has_item_r)
        {
          if (ShopHelper.IsTwoHanded(item_type))
          {
            if (ShopHelper.IsTwoHanded(item_type_l))
            {
              if (akimbo)
                return System.Tuple.Create(true, ActiveRagdoll.Side.RIGHT, text);
              else
      ...
      return System.Tuple.Create(true, equip_side, ShopHelper.IsActuallyTwoHanded(item_type) ? "BOTH" : text);
    }
