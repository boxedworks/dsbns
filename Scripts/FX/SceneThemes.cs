...
namespace Assets.Scripts.FX
{
  public class SceneThemes : MonoBehaviour
  {
    ...
    public static SceneTheme _Theme;
    ...
    static SceneTheme GetLevelTheme(int themeIter)
    {
    ...
    static IEnumerator ChangeMapThemeCo(SceneTheme oldtheme)
    {
      _ChangingTheme = true;
    ...
    return;
      ...
      var fields = typeof(SceneTheme).GetFields();
      foreach (var theme in _Instance._Themes)
      {
        outstring += $"new(){{\n";
        foreach (var field in fields)
        {
          ...
          if (field.FieldType == typeof(string)) outstring += $"\"{field.GetValue(theme)}\",\n";
          if (field.FieldType == typeof(bool)) outstring += $"{field.GetValue(theme).ToString().ToLower()},\n";
          if (field.FieldType == typeof(int)) outstring += $"{field.GetValue(theme)},\n";
          if (field.FieldType == typeof(float)) outstring += $"{field.GetValue(theme)}f,\n";
          if (field.FieldType == typeof(Color)) outstring += $"{GetColorString((Color)field.GetValue(theme))},\n";
        }
        outstring += $"}},\n";
      }
      // Print to console
    ...
    public static SceneTheme[] _Master_Themes = new SceneTheme[]
    {
new(){
